...
export default function ResortMap({
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800/80 rounded-3xl p-4 sm:p-5 shadow-2xl relative space-y-4 overflow-hidden w-full">
      ...
      <div className="relative w-full aspect-[1.8/1] min-h-[300px] sm:min-h-[360px] bg-slate-950 border-2 border-slate-900 rounded-2xl overflow-hidden select-none shadow-inner group">
        ...
        {buildings.map((b) => {
          ...
          return (
            <div
              key={b.id}
              className="absolute group/pin transition-all duration-300 transform -translate-x-1/2 -translate-y-1/2 z-10"
              style={{
                left: `${left}%`,
                top: `${top}%`,
              }}
            >
              ...
              <button
                ...
                className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 flex items-center justify-center font-mono font-bold text-[8.5px] cursor-pointer transition-all duration-300 hover:scale-130 active:scale-[0.9] text-center ${
                  getCategoryColor(b.resort, matchesFilter)
                } ${
                  isSelected 
                    ? 'scale-120 border-white ring-4 ring-amber-500/50 z-25 bg-amber-400 font-black' 
                    : !matchesFilter
        ...
        {(hoveredBuilding || selectedBuilding) && (
          ...
          >
            <div className="flex-1 text-right min-w-0">
              ...
              <strong className="text-xs text-white truncate inline-block vertical-middle font-bold">
